Shader "UShaderMagicBook/StdLight_SpotCookie"{
    Properties{
        _MainTex("Main Texture",2D) = "white"{}
        _BaseColor("Base Color",Color) = (1.0,1.0,1.0,1.0)
        _Gloss("Gloss",Range(8.0,200.0)) = 20.0
        _CookieScale("Cookie Scale",Range(0,100)) = 1
    }
    SubShader{
        Pass{
            ...
            fixed4 Pixel1(vertexOutput i):SV_TARGET{
                ...
                fixed3 specular = _LightColor0.xyz * pow(saturate(dot(halfDir,worldNormal)),_Gloss);
                    ...
                    //如果不是平行光，光照衰减可以用下面这种方式计算出来。
                        ...
                        float atten = (lightCoord.z > 0.05) * tex2D(_LightTexture0, lightCoord.xy/(lightCoord.w * _CookieScale) + 0.5).w * tex2D(_LightTextureB0,dot(lightCoord,lightCoord).rr).UNITY_ATTEN_CHANNEL;
                    #endif
                #endif
